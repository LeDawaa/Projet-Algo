...
void rec_NombreDe0ApresRetroPositionK(Liste l, int* k, int* acc) {
    (estVide(l -> suivant)) ? *acc = 0
                            : rec_NombreDe0ApresRetroPositionK(l -> suivant, k, acc);
    if (*k != 0) {
        *acc += ((l -> nombre) == 0);
        *k -= 1;
    }
...
void rec_sf_FctBegaye(Liste l, Liste* l2) {
    if (estVide(l)) return;
    if (l -> nombre > 0) {
        *l2 = ajoute(l -> nombre, ajoute(l -> nombre, NULL));
        l2 = &(((*l2) -> suivant) -> suivant);
    } return rec_sf_FctBegaye(l -> suivant, l2);
}

Liste sf_FctBegaye(Liste l) {
    Liste l2 = NULL;
    rec_sf_FctBegaye(l, &l2);
    return l2;
}
...
Liste iter_FctBegaye(Liste l) {
    ...
    while (!estVide(l)) {
        if (l -> nombre > 0) {
            if (p == NULL) {
                p = ajoute(l -> nombre, ajoute(l -> nombre, NULL));
                point = &((p -> suivant) -> suivant);
            } else {
                (*point) = ajoute(l -> nombre, ajoute(l -> nombre, NULL));
                point = &(((*point) -> suivant) -> suivant);
            }
        } l = l -> suivant;
    } return p;
}
...
void empile_bis(int x, BisListe *l) {
...
int main() {
    ...
    int nbListes = facto(number);
    int nbListe = nbListes * number;

    printf("Nombre suppose          : %d malloc de Liste | %d malloc de Listes.\n", nbListe, nbListes);

    clock_t begin1 = clock();
    Permutations_v2(number);
    double cpt_1 = cptListes;
    double cpt_3 = cptListe;
    clock_t end1 = clock();
    double elapsed1 = (double)(end1 - begin1);
    printf("Permutation basique     : %.2f s | %.f malloc de Liste | %.f malloc de Listes | %d free.\n", (elapsed1 / CLOCKS_PER_SEC), cpt_3, cpt_1, fcpt);

    /* On peut observer une diminution d'environ 30% des malloc de Liste.
    Mais on observe aussi près de 7 fois plus de malloc de Listes*/
    ...
    printf("Permutation optimisee   : %.2f s | %.f malloc de Liste | %.f malloc de Listes.\n", (elapsed2 / CLOCKS_PER_SEC), cpt_4, cpt_2);
    
    /* On peut observer que le nombre de malloc de Liste est extrêmement proche du nombre attendu.
    On observe aussi que le nombre de malloc de Listes est le même que le nombre de malloc attendu.*/

    printf("\nVersion optimisee %.2f%% plus rapide et %.2f%% moins de mallocs.\n\n", (((elapsed1 - elapsed2) / elapsed1) * 100), (((cpt_1 - cpt_2) / cpt_1) * 100));
    ...
    affiche_bis(bl);
    retire(&bl, 3);
    printf("\nL'element a l'index 3 est retire de la liste:\n\n");
    affiche_bis(bl);
